...
def dist_squared(t_q, t_i):
    ...
    for character in range(len(t_q)):
        sum += math.pow((t_q[character] - t_i[character]), 2.0)
    return sum

def hf(tq, ti, sigma):
    answer = 0.0
    answer = dist_squared(tq, ti)
    answer = answer * -1
    answer = answer / pow((2.0*sigma), 2.0)
...
def compute_fire_strengths(index, test_set, training_set, sigma):
    ...
    if (k != 0):
        temp_d = []
        for author in practice_set:
            new_index = author[-1]
            list_maker(temp_d, 12, 1, 1, new_index)
...
def compute_fire_strengths_CASIS25(index, test_set, training_set, sigma):
    ...
    if (k != 0):
        ...
        for author in practice_set:
            new_index = author[-1]
            list_maker(temp_d, 25, 1, 1, new_index)
